...
#ifdef _WIN32
const int ScreenWidth = static_cast<int>( GetSystemMetrics(SM_CXSCREEN) * 0.70 );
const int ScreenHeight = static_cast<int>(  GetSystemMetrics(SM_CYSCREEN) * 0.70 );
...
namespace ogl
{
	...
	protected:
		struct WindowInfo
		{
			WindowInfo():title("LearningOpenGL"), 
				        Width(ScreenWidth), 
						Height(ScreenHeight),
				        posX(PosX), 
						posY(PosY) {}
